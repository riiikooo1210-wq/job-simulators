...
export function workspaceAppVariant(kind: SourceWorkspaceApp['kind']) {
...
export function workspaceAppWindowTitle(app: SourceWorkspaceApp, ctx: BriefingContext) {
  if (app.kind === 'slack') {
...
export function SourceWorkspaceAppContent({
  app,
  workspaceFiles,
  activeFileId,
  visitedIds = [],
  ...
  const activeAppNeedsContentMargin = ['ticket', 'build_tracker', 'crm', 'ats'].includes(app.kind)
  const activeFileGroups = app.files ? groupSourceFiles(app.files) : []
  const activeAppHasFiles = Boolean(app.files?.length)
  ...
  return (
    ...
    >
      {!((app.kind === 'slack' || app.kind === 'email') && app.messages?.length) && (
        <div>
          <div style={{ fontSize: '0.72rem', color: '#3F605C', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0 }}>
            {app.kind.replace(/_/g, ' ')}
          </div>
          <h3 style={{ margin: '0.15rem 0 0', fontSize: '1rem', lineHeight: 1.3 }}>{app.title || app.label}</h3>
          {app.subtitle && (
            <div style={{ fontSize: '0.72rem', color: '#6f6758', marginTop: '0.2rem' }}>{interpolate(app.subtitle, ctx)}</div>
          )}
        </div>
      )}
      ...
      {app.messages && app.messages.length > 0 && (
        app.kind === 'slack' ? (
            ...
            {app.messages.map((message, i) => (
              <div key={message.id} style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                ...
                <SlackMessageEnhanced
                  ...
                  delay={i * 0.12}
                  initialExpanded
                  showUnreadDot={i === app.messages!.length - 1}
                />
                {showAttachments && (
          ...
          app.messages.map((message) => {
            ...
            if (message.channel === 'email' || app.kind === 'email') {
              return (
                <div key={message.id} style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                  {renderLookForCard(message.lookFor ? interpolate(message.lookFor, ctx) : undefined)}
                  <EmailBlock
                    email={resolveWorkspaceEmailMessage(message, ctx, app.title || app.label)}
                    initialExpanded
                  />
                  {showAttachments && (
                    <SourceAttachmentButtons
      ...
      {renderLookForCard(app.lookFor ? interpolate(app.lookFor, ctx) : undefined)}

      {app.sections?.map((section) => renderSourceFileSection({
        ...section,
        body: interpolate(section.body, ctx),
      }))}

      {renderWorkspaceRows(app, activeAppNeedsContentMargin)}
    </div>
  )
}
