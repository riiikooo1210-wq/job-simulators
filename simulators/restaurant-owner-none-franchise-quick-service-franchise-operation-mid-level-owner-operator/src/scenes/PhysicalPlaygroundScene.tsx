...
const LINE_CHECK_STANDARD_CARD_IMAGE = '/action-assets/scene_03_line_check/card.PNG'
const LINE_CHECK_STANDARD_REFERENCE_SUBJECTS = new Set([
  'hot_chicken_pan',
  'salsa_cold_pan',
  'sanitizer_bucket',
  'drive_thru_timer',
...
export default function PhysicalPlaygroundScene({ node }: { node: PlaygroundNode }) {
  ...
  const [showStandardCardReference, setShowStandardCardReference] = useState(false)
  ...
  const cleanLineCheckStage = node.id === 'scene_03_line_check'
  ...
  const inspect = (subject: {
    ...
    const image = getAssetPath(subject, lookup)
    ...
    setShowStandardCardReference(false)
    setModal({
      title: subject.label,
      body,
      image,
  ...
  const completeInspection = (current: ModalContent, answer?: boolean) => {
    ...
    if (current.requiresObservationText && node.id !== 'scene_03_line_check' && observation.split(/\s+/).filter(Boolean).length < 3) {
      warn(`Write the observation for ${current.title} before submitting it.`)
      return
    }
  ...
  const canShowStandardCardReference = cleanLineCheckStage
    && Boolean(modal)
    && LINE_CHECK_STANDARD_REFERENCE_SUBJECTS.has(modal?.subjectId || '')
  ...
  return (
    <SceneWrapper showBack backLabel="Back" hideIllustration>
      ...
      >
        ...
        <section className={`physical-playground ${cleanLineCheckStage ? 'physical-playground--edge-to-edge physical-playground--clean-stage' : ''}`}>
          <div className="physical-playground-stage" aria-label={`${node.title} workplace stage`}>
            ...
            {!cleanLineCheckStage && import.meta.env.DEV && (node.background?.coordinateMap || []).map((zone) => (
              <div
                key={zone.label}
            ...
            {derived.targets.map((target, index) => {
              ...
              const placedObjects = Object.entries(state.placements)
                ...
                .filter(Boolean) as ActionInteractiveObject[]
              return (
                <div
                  key={target.id}
                ...
                >
                  {!cleanLineCheckStage && assetPath && <GeneratedAssetImage src={assetPath} label={target.label} compact />}
                      ...
                      {placedObjects.map((object) => {
                        ...
                        return (
                          <span key={object.id} className="physical-playground-placed-item">
                            {!cleanLineCheckStage && objectAssetPath && !isObjectConsumed(object.id) && <GeneratedAssetImage src={objectAssetPath} label={object.label} compact />}
                            <b>{object.label}</b>
                          </span>
                        )
                      })}
            ...
            {derived.readables.map((surface, index) => {
              ...
              return (
                <button
                  ...
                  aria-label={surface.label}
                >
                  {!cleanLineCheckStage && assetPath && <GeneratedAssetImage src={assetPath} label={surface.label} compact />}
                  <span className="physical-playground-object-label">
                    <b>{done ? 'Inspected' : 'Inspect'}</b>
                    {surface.label}
                  </span>
                </button>
              )
            ...
            {derived.objects.filter((object) => !isObjectConsumed(object.id)).map((object, index) => {
              ...
              return (
                ...
                >
                  {!cleanLineCheckStage && assetPath && <GeneratedAssetImage src={assetPath} label={object.label} compact />}
                  <span className="physical-playground-object-label">
                    <b>{selected && (role === 'readable' || object.readableText) ? 'Inspected' : hint}</b>
                    {object.label}
                  </span>
                </button>
              )
        ...
        {modal && (
          <div className="physical-playground-modal-backdrop" onClick={() => setModal(null)}>
            <div className="physical-playground-modal" onClick={(e) => e.stopPropagation()}>
              ...
              {!!latestWarning && <WarningCallout message={latestWarning} />}
              {modal.image && !modalImageFailed && modal.image.includes('line_check_standard_closeup') && (
                <div className="physical-playground-modal-image-crop physical-playground-modal-image-crop--line-standard">
                  <img
                    src={modal.image}
                    alt=""
                    onError={(e) => {
              ...
              {canShowStandardCardReference && (
                <div className="physical-playground-standard-reference">
                  <button
                    type="button"
                    onClick={() => setShowStandardCardReference((visible) => !visible)}
                  >
                    {showStandardCardReference ? 'Hide standard card' : 'Show standard card'}
                  </button>
                  {showStandardCardReference && (
                    <img src={LINE_CHECK_STANDARD_CARD_IMAGE} alt="Line-check standard card" />
                  )}
                </div>
              )}
