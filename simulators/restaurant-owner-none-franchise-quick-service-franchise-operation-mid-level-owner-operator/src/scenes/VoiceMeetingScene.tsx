...
function formatStructuredPrepNote(raw: string): string {
  try {
    ...
    const labels: Record<string, string> = {
      ...
      jordan_action: 'Jordan action before lunch',
      impact: 'KPI / standard impact',
    }
    ...
    return parsed
      .map((item, idx) => {
        ...
        return lines.length ? `Coaching item #${idx + 1}\n${lines.join('\n')}` : ''
      })
      .filter(Boolean)
      .join('\n\n') || raw
  } catch {
...
export default function VoiceMeetingScene({ node }: Props) {
  ...
  const goalPrompt = interpolate(node.goalPrompt, interpolationContext)
  const meetingContext = interpolate(node.meetingContext || node.content || '', interpolationContext)
  const playerGoal = node.playerGoal ? interpolate(node.playerGoal, interpolationContext) : ''
  const endpoint = node.endpoint ? interpolate(node.endpoint, interpolationContext) : ''
  const successCriteria = node.successCriteria ? interpolate(node.successCriteria, interpolationContext) : ''
  const prepReference = node.prepReferenceContent
    ? interpolate(node.prepReferenceContent, interpolationContext)
    : ''
  const prepNoteRaw = node.prepNoteKey ? freeTextResponses[node.prepNoteKey]?.trim() : ''
  const prepNote = prepNoteRaw ? formatStructuredPrepNote(prepNoteRaw) : ''
