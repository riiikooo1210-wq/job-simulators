...
function hasDailyTableTarget(raw: string | undefined): boolean | null {
  if (!raw) return null
  try {
    const parsed = JSON.parse(raw)
    if (!Array.isArray(parsed) || parsed.length === 0) return null
    const reporters = parsed
      .map((item) => String(item?.reporter || '').trim())
      .filter(Boolean)
    if (reporters.length === 0) return null
    return reporters.some((reporter) => {
      return reporter.includes('The Daily Table') || reporter.includes('Lena Ortiz')
    })
  } catch {
    return null
  }
...
export default function FreeTextScene({ node }: Props) {
  ...
  const dailyTableSelected = node.id === 'pitch_email' ? hasDailyTableTarget(responses.media_targets) : null
  const content = node.id === 'pitch_email' && dailyTableSelected === false
    ? 'Even though you did not include The Daily Table on the list, Maya asks you to draft the first tailored pitch to Lena Ortiz at The Daily Table. Review the Launch Facts, Reporter Context, and Pitch Guidance tabs in the email window before writing. Keep it short, relevant, and fact-safe. A pitch is not a full press release.'
    : node.content
