...
function buildSystemPrompt(args: {
  ...
  const { node, npc, playerName, goalPrompt, meetingContext, prepReference, prepNote } = args
  ...
  const initial = (node.initialMessages || [])
    ...
    .join('\n')
  const journalistCallProtocol = node.id === 'journalist_call'
    ? `JOURNALIST CALL TURN PROTOCOL:
- This call has exactly four Lena questions and four student answer rounds.
- The first NPC message is Question 1. It has already been shown before the student starts talking. Do not repeat it.
- After the student's first answer, ask Question 2 about the sweetener or lower-sugar language.
- After the student's second answer, ask Question 3 about the tree-nut and facility language.
- After the student's third answer, ask Question 4 about materials, spokesperson access, or exact follow-up.
- After the student's fourth answer, briefly close the call. Do not ask a fifth question.
- Ask exactly one question per NPC turn. If you need to challenge an overclaim, do it in one short sentence before the next scheduled question.`
    : ''
...
${prepNote ? `STUDENT'S PRIOR PITCH DRAFT:
${prepNote}

Use the student's pitch draft as the reason for this follow-up call. Ask about the angle, claims, omissions, or confusing wording that actually appear in the draft. If the draft is vague, ask what makes the story useful for readers. If it overclaims, challenge the exact claim. If it omits sweetener or allergen details, ask for those details rather than pretending they were already included.
` : ''}
${prepReference ? `APPROVED CALL FACTS:
${prepReference}

Use these facts to evaluate the student's answers, but do not read them aloud as a script.
` : ''}
${initial ? `INITIAL CONTEXT MESSAGES:\n${initial}\n` : ''}
${journalistCallProtocol ? `${journalistCallProtocol}\n` : ''}
RULES:
- ${speechStyle}
...
function buildPitchFollowUpMessage(pitch: string, playerName: string): string {
  const name = playerName || 'there'
  const lower = pitch.toLowerCase()
  const bannedClaim = ['healthy', 'clean', 'detox', 'guilt-free', 'allergen-free', 'safe for everyone', 'no risk']
    .find((term) => lower.includes(term))
  if (bannedClaim) {
    const article = bannedClaim === 'allergen-free' ? 'an' : 'a'
    return `Hi ${name}, I read your note and have about ten minutes before edit. I have four questions, one answer each. First: you used language that sounds like ${article} ${bannedClaim} claim. What exactly can Northstar substantiate about Luma Fizz?`
  }
  if (/allerg|tree nut|facility|safe for readers with nut/i.test(pitch)) {
    return `Hi ${name}, I read your note and have about ten minutes before edit. I have four questions, one answer each. First: you brought up the facility and tree-nut language, so why did you include that in the pitch?`
  }
  if (/monk fruit|sweetener|fruit juice|low-sugar|35 calories/i.test(pitch)) {
    return `Hi ${name}, I read your note and have about ten minutes before edit. I have four questions, one answer each. First: you leaned on the sweetener and lower-sugar angle. Why is that the right story for my readers?`
  }
  if (/ingredient|transparent|label|grocery|beverage/i.test(pitch)) {
    return `Hi ${name}, I read your note and have about ten minutes before edit. I have four questions, one answer each. First: you framed this around grocery and ingredient transparency. Why is Luma Fizz worth reader attention beyond another big-brand beverage launch?`
  }
  return `Hi ${name}, I read your note and have about ten minutes before edit. I have four questions, one answer each. First: your pitch was pretty broad, so why is Luma Fizz worth reader attention?`
}
...
export default function VoiceMeetingScene({ node }: Props) {
  ...
  const initialMessages = node.id === 'journalist_call' && prepNote
    ? [{ role: 'npc' as const, content: buildPitchFollowUpMessage(prepNote, playerName) }]
    : (node.initialMessages || []).map((m) => ({
      ...m,
  ...
  useEffect(() => {
    ...
    if (initialMessages.length && messages.length === 0) {
      initialMessages.forEach((m) => appendNpcMessage(conversationKey, m))
    }
  }, [appendNpcMessage, conversationKey, initialMessages, messages.length])
  ...
  useEffect(() => {
    if (userTurns >= maxTurns && status === 'connected' && !meetingEnded && !npcSpeaking && !liveNpc.trim()) {
      endMeeting()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userTurns, maxTurns, status, meetingEnded, npcSpeaking, liveNpc])
