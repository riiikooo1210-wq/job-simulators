...
const HAND_HYGIENE_SAFETY_CRITERION = '部屋512観察'
...
7. Treat structured physical-playground warnings and actionLog entries as grading evidence. If Scene 1B / scene2_assessment includes "${HAND_HYGIENE_FIRST_WARNING}" or a safety_warning before hand hygiene, do not award full credit for "${HAND_HYGIENE_SAFETY_CRITERION}".
8. For scene2_assessment pain-score documentation, accept any pain score from 8/10 through 9/10 inclusive, including "8", "8.5", "9", "8-9/10", or "between 8 and 9"; do not penalize the candidate for not choosing exactly 8/10 if they capture severe sharp hip pain and/or worsening with turning.
9. For scene2_assessment wound observation, accept documentation that mentions swelling/腫れ or redness/赤み around the wound/dressing, including plain-language phrases such as "swollen", "red", "腫れてる", or "赤い". If the candidate captures at least one of those concepts, do not penalize them for omitting dry/intact status, drainage, or no-spreading-redness details.
10. Write a 2–4 sentence overall_assessment grounded in the candidate's actual path and decisions.
...
function hasHandHygieneFirstViolation(state: SerializedState): boolean {
  const raw = state.freeTextResponses[HAND_HYGIENE_FIRST_SCENE_ID]
  if (!raw) return false
  try {
    const parsed = JSON.parse(raw)
    const warnings = Array.isArray(parsed.warnings) ? parsed.warnings : []
    const actionLog = Array.isArray(parsed.actionLog) ? parsed.actionLog : []
    return warnings.includes(HAND_HYGIENE_FIRST_WARNING)
      || actionLog.some((entry: any) => (
        entry?.action === 'safety_warning'
        && typeof entry.label === 'string'
        && entry.label.toLowerCase().includes('before hand hygiene')
      ))
  } catch {
    return raw.includes(HAND_HYGIENE_FIRST_WARNING)
  }
}

function recommendationFor(scorePercentage: number): GradingResult['recommendation'] {
...
function recomputeTotals(result: GradingResult): GradingResult {
  const total = result.scenarios.reduce((sum, scenario) => (
    sum + scenario.criteria.reduce((inner, criterion) => inner + criterion.score, 0)
  ), 0)
  const max = result.max_score || buildResponseTemplate().maxScore
  const scorePercentage = Number((total / max).toFixed(2))
  return {
    ...result,
    total_score: total,
    max_score: max,
    score_percentage: scorePercentage,
    recommendation: recommendationFor(scorePercentage),
  }
}

function applyDeterministicGradingRules(result: GradingResult, state: SerializedState): GradingResult {
  if (!hasHandHygieneFirstViolation(state)) return result

  const safetyCriterion = result.scenarios
    .flatMap((scenario) => scenario.criteria)
    .find((criterion) => criterion.criterion === HAND_HYGIENE_SAFETY_CRITERION)
  if (!safetyCriterion) return result

  const mentionsHandHygiene = /hand hygiene|sanitize/i.test(safetyCriterion.comment)
  if (safetyCriterion.score >= rubric.max_score_per_criterion || !mentionsHandHygiene) {
    safetyCriterion.score = Math.max(0, safetyCriterion.score - 1)
  }
  if (!mentionsHandHygiene) {
    safetyCriterion.comment = `${safetyCriterion.comment.replace(/\.$/, '')}; the action log also shows they began a room task before hand hygiene.`
  }
...
export async function gradeResponses(state: SerializedState): Promise<AssessmentResult> {
  ...
  const legacy = applyDeterministicGradingRules(parsed, state)
  return buildAssessmentResult(legacy, rubric, state)
}
