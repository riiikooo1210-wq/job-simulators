...
export function BriefingDrawerContent({ node }: { node: BriefingNode }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      {node.referenceTitle && node.referenceContent && (
        <ReferenceCard title={node.referenceTitle} content={interpolate(node.referenceContent, ctx)} />
      )}
      {allSteps.length === 0 && (
        <>
          {node.slackMessages?.map((msg, i) => (
            <SlackMessageEnhanced key={i} message={{ ...msg, content: interpolate(msg.content, ctx) }} delay={0} showUnreadDot={i === (node.slackMessages?.length ?? 0) - 1} />
          ))}
      ...
      {allSteps.map((s, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingTop: i > 0 ? '0.75rem' : 0, borderTop: i > 0 ? '1px solid rgba(0,0,0,0.12)' : 'none' }}>
          ...
          {s.slackMessages?.map((msg, j) => (
            <SlackMessageEnhanced key={j} message={{ ...msg, content: interpolate(msg.content, ctx) }} delay={0} showUnreadDot={j === (s.slackMessages?.length ?? 0) - 1} />
          ))}
...
function ReferenceCard({ title, content }: { title: string; content: string }) {
  ...
  const flushBullets = () => {
    ...
    blocks.push(
      <ul key={`bullets-${blocks.length}`} style={{ margin: 0, paddingLeft: '1.5rem', listStyleType: 'disc', listStylePosition: 'outside', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
        {bullets.map((bullet, i) => (
          <li key={i} style={{ paddingLeft: '0.25rem' }}>
  ...
  lines.forEach((rawLine) => {
    ...
    if (heading) {
      blocks.push(
        <div key={`heading-${blocks.length}`} style={{ fontSize: '0.8125rem', fontWeight: 800, color: '#3A6B5E', paddingTop: blocks.length > 0 ? '0.35rem' : 0 }}>
          {heading[1]}
        </div>
      )
      return
  ...
  return (
    ...
    >
      <div style={{ fontSize: '0.875rem', fontWeight: 800, color: '#000' }}>{title}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', fontSize: '0.875rem', lineHeight: 1.6, color: '#222' }}>
        {blocks}
      </div>
    </section>
  )
