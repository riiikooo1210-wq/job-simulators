...
| 8 | `scene_02_problem_brief` | structured_entry | Refine analytics and current-app observations into four PM problem notes. | In-window tabs: Analytics @ scene_02_problem_brief.workSurface.sourceTabs.amplitude_funnel; Current App Audit @ scene_02_problem_brief.workSurface.sourceTabs.current_app_audit | Problem notes (structured_entry) | Evidence-based problem framing; Risk and unknown identification; Clarity under ambiguity; structuredEntries.problemBrief | Static next checked: transition_research; runtime DOM checked for tabs | No high-risk visual surface. | Source tabs render only analytics and audit observations inside the work window. |
