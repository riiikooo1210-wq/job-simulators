...
function SentMessagePreview({
  ...
  if (appWindow === 'email') {
    return (
      ...
      >
        <div style={{ padding: '0.75rem 0.875rem', borderBottom: '1px solid #E5E7EB', backgroundColor: '#F8FAFC' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '3.5rem 1fr', rowGap: '0.25rem', columnGap: '0.5rem', fontSize: '0.75rem', color: '#4B5563' }}>
            <span style={{ fontWeight: 700, color: '#6B7280' }}>From</span>
            <span>{displayName}</span>
            <span style={{ fontWeight: 700, color: '#6B7280' }}>To</span>
            <span>Recipient</span>
            <span style={{ fontWeight: 700, color: '#6B7280' }}>Subject</span>
            <span>{subject || 'Selected reply'}</span>
          </div>
        </div>
        <div style={{ padding: '0.875rem', display: 'flex', gap: '0.625rem' }}>
          <div
            style={{
              width: '34px',
              height: '34px',
...
export default function MultipleChoiceScene({ node }: Props) {
  ...
  const appWindow = node.appWindow || node.workSurface ? resolveWorkSurfaceVariant(node, 'slack') as LaptopFrameVariant : undefined
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: appWindow ? '1rem' : 0 }}>
      ...
      {selectedOption && isMessageReplyScene && (
        <SentMessagePreview
          ...
          subject={node.windowTitle || node.workSurface?.title || node.title}
        />
      )}
      {promptText && (
        <div
  ...
  return (
    <SceneWrapper illustration={node.illustration} showBack backLabel="Back" hideIllustration={hasWorkSurfaceVisual(node)}>
      ...
      >
        ...
        {appWindow ? (
          <WorkSurfaceFrame node={node} variant={appWindow} title={node.workSurface?.title || node.windowTitle || node.title}>
            {choiceContent}
          </WorkSurfaceFrame>
        ) : (
          choiceContent
