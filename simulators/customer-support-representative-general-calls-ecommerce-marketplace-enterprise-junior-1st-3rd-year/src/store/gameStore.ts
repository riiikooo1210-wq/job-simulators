...
const isValidNodeId = (nodeId: unknown): nodeId is string =>
  typeof nodeId === 'string' && Boolean(storyline.nodes[nodeId])

function sanitizePersistedState(persistedState: unknown): Partial<GameState> {
  if (!persistedState || typeof persistedState !== 'object') return initialState

  const persisted = persistedState as Partial<GameState>
  if (!isValidNodeId(persisted.currentNodeId)) return initialState

  const visitedNodes = Array.isArray(persisted.visitedNodes)
    ? persisted.visitedNodes.filter(isValidNodeId)
    : [persisted.currentNodeId]

  return {
    ...persisted,
    visitedNodes: visitedNodes.length ? visitedNodes : [persisted.currentNodeId],
    currentSection: storyline.nodes[persisted.currentNodeId].section ?? 0,
  }
}
