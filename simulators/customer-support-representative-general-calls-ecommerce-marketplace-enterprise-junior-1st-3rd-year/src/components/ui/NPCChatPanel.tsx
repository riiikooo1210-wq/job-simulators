...
function formatSlackTimestamp(ts?: string): string {
  if (!ts) return 'Now'
  if (/\b(?:AM|PM)\b/i.test(ts)) return ts
...
export default function NPCChatPanel({
  ...
  const isSlackThread = channel === 'slack'
  const displayName = playerName.trim() || 'You'

  const messageList = isSlackThread ? (
    <div
      ref={scrollRef}
      style={{
        display: 'flex',
    ...
    >
      ...
      {messages.map((m, i) => (
        <SlackMessageEnhanced
          key={i}
          message={{
            sender: m.role === 'user' ? displayName : (m.npcName ?? npc.name),
            role: m.role === 'user' ? 'You' : npc.role,
            timestamp: formatSlackTimestamp(m.ts),
            content: m.content,
            avatarInitials: m.role === 'user' ? getPlayerInitials(playerName) : undefined,
          }}
          initialExpanded
        />
      ))}
    ...
    >
      {messages.length === 0 && (embedded ? playerGoal : goalPrompt) && (
        <p style={{ fontStyle: 'italic', color: '#888', fontSize: '0.8125rem' }}>{embedded ? playerGoal : goalPrompt}</p>
      )}
      {messages.map((m, i) => (
        <motion.div
          key={i}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      {!presetsOnly && (
        <SlackCompose
          ...
          showChannelHeader={!(embedded && isSlackThread)}
        />
      )}
