...
export default function KanbanBoardScene({ node }: { node: KanbanBoardNode }) {
  ...
  const isDropdownClassification = node.classificationMode === 'dropdown'
  const categoryColumns = node.columns.filter((col) => col.id !== 'inbox')
  const titleTabs = [
    { id: 'board', label: node.windowTitle ?? node.title },
    ...(node.referenceContent ? [{ id: 'reference', label: node.referenceTitle || 'Reference' }] : []),
    { id: 'rationale', label: 'Screening notes' },
  ]

  const dropdownBoardContent = (
    <div style={{ height: '100%', minHeight: 0, overflowY: 'auto', padding: '0.875rem', backgroundColor: '#F7F1E3' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
        ...
        >
          <div style={{ fontSize: '0.8125rem', fontWeight: 800, color: '#3F605C' }}>
            {node.assignmentTitle || 'Company category assignments'}
          </div>
          <div style={{ fontSize: '0.72rem', color: '#5F543F', fontWeight: 700 }}>
            {movedCount}/{node.cards.length} selected
          </div>
        </div>

        {node.cards.map((card) => {
          const selectedColumn = cardState[card.id] ?? card.initialColumn
          return (
            <div
              key={card.id}
              style={{
            ...
            >
              <div style={{ minWidth: 0, color: '#1E1E1A', fontSize: '0.82rem', lineHeight: 1.35, overflowWrap: 'anywhere' }}>
                {card.text}
              </div>
              <select
                aria-label={`Category for ${card.text}`}
                value={selectedColumn === card.initialColumn ? '' : selectedColumn}
                onChange={(event) => {
                  const updated = {
                    ...cardState,
                    [card.id]: event.target.value || card.initialColumn,
                  }
                  setFreeTextResponse(node.bindingKey, serializeKanban(updated))
                }}
                style={{
                  ...
                  minWidth: 0,
                  border: '1px solid #CDBF94',
                  backgroundColor: selectedColumn === card.initialColumn ? '#FBF7EA' : '#EFE8D2',
                  color: '#1E1E1A',
                  borderRadius: '4px',
  ...
  const boardContent = isDropdownClassification ? dropdownBoardContent : dragBoardContent
      ...
      <div style={{ border: '1px solid #CDBF94', backgroundColor: '#FBF7EA', padding: '0.75rem', borderRadius: '6px', display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
        <div style={{ fontSize: '0.7rem', color: '#3F605C' }}>
          {node.requireAllCardsMoved
            ? `${movedCount}/${node.cards.length} ${isDropdownClassification ? 'categories chosen' : `card${node.cards.length !== 1 ? 's' : ''} sorted`}`
        ...
        {movedCount < requiredMovedCount && (
          <span style={{ fontSize: '0.7rem', color: '#3F605C' }}>
            {node.requireAllCardsMoved
              ? isDropdownClassification
                ? `Choose categories for all ${node.cards.length} alternatives before submitting.`
  ...
  const referenceContent = (
    <div style={{ height: '100%', minHeight: 0, overflowY: 'auto', padding: '0.875rem', backgroundColor: '#F7F1E3', color: '#1E1E1A' }}>
      <div
        style={{
          border: '1px solid #CDBF94',
          backgroundColor: '#FBF7EA',
    ...
    <SceneWrapper illustration={node.illustration} showBack backLabel="Back">
      ...
      >
        ...
        <DesktopOverlay>
          ...
          >
            {activeAppTab === 'rationale' ? rationaleContent : activeAppTab === 'reference' ? referenceContent : boardContent}
          </LaptopFrame>
        </DesktopOverlay>
      </motion.div>
