...
export default function VoiceMeetingScene({ node }: Props) {
  ...
  const sessionNotesBox = sessionNotesKey ? (
    <div
      style={{
        background: '#FBF7EA',
    ...
    >
      <div>
        <div style={{ fontSize: '0.8125rem', fontWeight: 800, color: '#1E1E1A' }}>
          {node.sessionNotesTitle || 'Meeting notes'}
        </div>
        {node.sessionNotesPrompt && (
          <div style={{ marginTop: '0.25rem', fontSize: '0.75rem', lineHeight: 1.5, color: '#555' }}>
            {renderContentWithGlossary(interpolate(node.sessionNotesPrompt, { playerName, branchFlags, mcSelections }))}
          </div>
        )}
      </div>
      <textarea
        ...
        placeholder={node.sessionNotesPlaceholder || 'Capture facts, quotes, and follow-ups you may need after the meeting.'}
        rows={4}
        style={{
          width: '100%',
  ...
  const referenceContent = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
      {node.sessionNotesPlacement === 'reference' && sessionNotesBox}
      {node.sessionNotesPlacement === 'reference' && sessionNotesBox && (prepNote || prepReference) && (
          ...
          {!canSubmit && (
            <span style={{ fontSize: '0.75rem', color: '#666' }}>
              {minTurns > 0
                ...
                : `End the ${interactionLabel} when you are ready to continue.`}
            </span>
          )}
        </div>
      </motion.div>
