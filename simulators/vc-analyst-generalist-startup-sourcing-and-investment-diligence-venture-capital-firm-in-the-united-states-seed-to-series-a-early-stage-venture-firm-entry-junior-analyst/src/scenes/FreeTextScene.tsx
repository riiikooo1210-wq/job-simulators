...
export default function FreeTextScene({ node }: Props) {
  ...
  const sceneIllustration = node.illustration || (node.id === 'founder_call_prep' ? '/scenes/founder_call_prep.png' : undefined)
        ...
        <div style={{ padding: '0.75rem 1rem 0.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          {node.slackMessages.map((message, index) => (
            <SlackMessageEnhanced
              ...
              delay={0}
              showUnreadDot={index === (node.slackMessages?.length ?? 0) - 1}
            />
          ))}
        </div>
      )}
