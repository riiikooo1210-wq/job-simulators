...
import type { Node, Edge, Connection, OnConnect, NodeChange } from '@xyflow/react'
...
// ---- Types ----
type NodeData = { label: string; locked?: boolean }
...
const START_NODE_ID = 'friend_streaks_home'
const START_NODE_LABEL = 'Friend Streaks Home'
const START_NODE_POSITION = { x: 80, y: 60 }
const DEFAULT_EDGE_LABEL = 'Button label'

function makeStartNode(position = START_NODE_POSITION): FDNode {
  return {
    id: START_NODE_ID,
    type: 'screen',
    position,
    data: { label: START_NODE_LABEL, locked: true },
    deletable: false,
  }
}
  ...
  return (
    <div
      ...
      style={{
        minWidth: 130,
        padding: '8px 14px',
        background: locked ? '#E6EFE3' : selected ? '#FFF5E0' : '#fff',
        border: `2px solid ${locked ? '#3A6B5E' : selected ? '#B87D6B' : '#000'}`,
        boxShadow: selected ? '3px 3px 0 #B87D6B' : `3px 3px 0 ${locked ? '#3A6B5E' : '#000'}`,
        textAlign: 'center',
        fontSize: 13,
    ...
    >
      ...
      {editing ? (
        ...
        <span>
          {locked && (
            <span style={{ display: 'block', color: '#3A6B5E', fontSize: 10, fontWeight: 900, lineHeight: 1.1, textTransform: 'uppercase' }}>
              Start
            </span>
          )}
          {data.label}
...
export default function FlowDiagramScene({ node }: { node: FlowDiagramSceneNode }) {
  ...
  const { initNodes, initEdges } = useMemo(() => {
    try {
      ...
      const startSaved = saved.nodes.find((n) => n.id === START_NODE_ID || n.label.trim().toLowerCase() === START_NODE_LABEL.toLowerCase())
      const idMap = new Map<string, string>()
      if (startSaved && startSaved.id !== START_NODE_ID) idMap.set(startSaved.id, START_NODE_ID)
      const shouldShiftExisting = !startSaved && saved.nodes.length > 0
      const nonStartNodes = saved.nodes.filter((n) => n !== startSaved)
      const hydratedNodes: FDNode[] = [
        makeStartNode(startSaved ? { x: startSaved.x, y: startSaved.y } : START_NODE_POSITION),
        ...nonStartNodes.map((n) => ({
          id: n.id,
          type: 'screen',
      ...
      return { initNodes: [makeStartNode()], initEdges: [] as FDEdge[] }
    }
  ...
  const persist = useCallback((nodes: FDNode[], edges: FDEdge[]) => {
    const normalizedNodes = nodes.some((n) => n.id === START_NODE_ID)
      ? nodes.map((n) => n.id === START_NODE_ID ? { ...n, data: { ...n.data, label: START_NODE_LABEL, locked: true }, deletable: false } : n)
      : [makeStartNode(), ...nodes]
    ...
    const data: SerializableFlow = {
      nodes: normalizedNodes.map((n) => ({ id: n.id, label: n.data.label, x: n.position.x, y: n.position.y })),
  ...
  const onNodesChange = useCallback((changes: NodeChange<FDNode>[]) => {
    const safeChanges = changes.filter((change) => !(change.type === 'remove' && change.id === START_NODE_ID))
  ...
  const applyEdgeLabel = useCallback(() => {
    if (!selEdgeId) return
    const label = labelDraft.trim() || DEFAULT_EDGE_LABEL
  ...
  const canvasAndControls = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: appWindow ? '0.75rem' : '0' }}>
      ...
      <div style={{ fontSize: '0.75rem', lineHeight: 1.6, background: '#F2EBD9', color: '#555', border: '1px solid #ccc', padding: '0.4rem 0.6rem' }}>
        <strong>How to use:</strong> Friend Streaks Home is the fixed starting screen. Click <strong>+ Add Screen</strong> to add the next screen. Drag right handle → left handle to draw an arrow. Label each arrow with the button, link, or CTA label the user chooses, like <strong>Send Invite</strong>. Double-click a screen to rename.
      </div>
      ...
      {selEdge && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.75rem', background: '#FFF5E0', border: '1px solid #000' }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#444', whiteSpace: 'nowrap' }}>Button/link label:</span>
          <input
            autoFocus
            value={labelDraft}
            ...
            placeholder="e.g. Send Invite, Accept Invite, View Shared Streak"
            style={{
              flex: 1,
              padding: '4px 8px',
              fontSize: '0.8125rem',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
        ...
        <textarea
          ...
          placeholder="e.g. I started at Friend Streaks Home so users can see the shared habit context first. The invite step is lightweight to reduce friction. I included an empty state because first-time users will have no friends yet..."
          rows={3}
          style={{
