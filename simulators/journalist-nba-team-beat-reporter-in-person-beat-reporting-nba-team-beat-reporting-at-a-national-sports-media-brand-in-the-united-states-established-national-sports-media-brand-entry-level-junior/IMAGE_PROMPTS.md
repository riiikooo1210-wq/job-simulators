...
First-person view from an NBA arena media-row work table before tipoff. A laptop shows a CMS draft, live box score, Slack editor thread, and credential badge for Harbor City Arena; a notebook and recorder sit beside the keyboard while the court glows in the background.
...
Scene-specific subject:
Arena media room laptop with Slack assignment messages, official injury report, arena media schedule, stats dashboard, social media feed, CMS sidebar, deadline timer, and live game credential. The basketball court is visible through an open tunnel.
...
## redirect_strong_angle
Filename: public/scenes/redirect_strong_angle.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
Slack thread in an NBA arena media room where the editor greenlights a focused Reed-return and closing-lineup angle.
...
## redirect_rumor
Filename: public/scenes/redirect_rumor.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
Phone screen full of rumor posts beside a reporter notebook marked no confirmation. Editor Slack message says verify before amplifying.
...
## redirect_generic
Filename: public/scenes/redirect_generic.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
A generic game preview draft fading behind a sharper sticky note: Reed return plus closing lineup.
...
## redirect_color
Filename: public/scenes/redirect_color.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
Coach at podium in a tense pregame media room, with reporter notes connecting mood to lineup decisions rather than gossip.
...
## warmup_observation
Filename: public/scenes/warmup_observation.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
Arena media-row view before an NBA game at Harbor City Arena. Foreground has a reporter work table with credential, notebook, phone, and a laptop open to a team PR injury-status email; midground shows Malik Reed warming up near the three-point line; a trainer stands near the baseline; empty lower-bowl seats and court lights fill the background. Leave clear clickable zones for the laptop PR email, phone feed, Reed movement, and trainer check-in.

Coordinate map (percent coordinates inside the 16:9 image):
- media row work table: x 5%, y 58%, w 38%, h 30%
- team PR injury email on laptop: x 85%, y 58%, w 15%, h 12%
- phone rumor feed: x 72%, y 68%, w 9%, h 15%
- Reed warmup movement: x 48%, y 32%, w 16%, h 24%
- trainer baseline check-in: x 66%, y 34%, w 13%, h 20%
...
## coach_availability
Filename: public/scenes/coach_availability.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
In-person NBA pregame interview room. Coach Len Harris stands at a small riser with team backdrop while reporters hold recorders and phones. First-person reporter perspective, no laptop or video call grid.
...
## media_scrum
Filename: public/scenes/media_scrum.png

COPY THIS WHOLE PROMPT.

Scene-specific subject:
In-person NBA postgame media scrum near the baseline. Reporters hold phones and recorders toward veteran guard Malik Reed in team gear, with court lights, camera operators, and the tunnel behind him. No video call grid or laptop screen.
...
Scene-specific subject:
Quiet arena media room after deadline. A reporter laptop shows a submitted story, Slack is quiet, and the empty court is visible beyond the seats.
...
Scene-specific subject:
Depict the moment described in the scene titled Final Report.
