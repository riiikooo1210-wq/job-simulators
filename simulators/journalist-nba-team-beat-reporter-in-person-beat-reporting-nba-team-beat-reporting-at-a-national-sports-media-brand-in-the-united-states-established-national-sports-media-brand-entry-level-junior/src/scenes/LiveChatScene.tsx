...
export default function LiveChatScene({ node }: Props) {
  ...
  const contextResponses = (node.contextResponseKeys || []).map((entry) => ({
    key: entry.key,
    title: entry.title || humanizeKey(entry.key),
    content: freeTextResponses[entry.key] || '',
  }))
  ...
  const chatPanel = (
    <NPCChatPanel
      ...
      aiInitialMessagePrompt={node.aiInitialMessagePrompt}
      aiInitialMessageFallback={node.aiInitialMessageFallback}
