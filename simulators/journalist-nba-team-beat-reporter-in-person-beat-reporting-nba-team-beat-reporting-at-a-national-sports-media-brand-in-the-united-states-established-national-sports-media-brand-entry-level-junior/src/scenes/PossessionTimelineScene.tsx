...
export default function PossessionTimelineScene({ node }: { node: PossessionTimelineNode }) {
  ...
  const currentNote = state.notes[currentEvent.id] || { note: '' }
  ...
  const questionPlaceholders = questionIndexes.map((index) => node.questionPlaceholders?.[index] || 'Write a concise, specific question for Reed.')
            ...
            >
              ...
              {!canSubmit && (
                <span style={{ fontSize: '0.75rem', color: '#666' }}>
                  Watch all possessions, save at least {minNotes} useful notes, and write something in each Reed question box.
                </span>
              )}
