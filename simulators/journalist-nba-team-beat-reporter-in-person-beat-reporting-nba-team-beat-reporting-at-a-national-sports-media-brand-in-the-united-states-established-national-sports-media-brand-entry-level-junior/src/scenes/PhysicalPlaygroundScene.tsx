...
const RUNNING_NOTES_KEY = '__running_reporter_notes'
...
export default function PhysicalPlaygroundScene({ node }: { node: PlaygroundNode }) {
  ...
  const warn = (message: string) => {
    save({ ...state, warnings: unique([...state.warnings, message]) })
  }
  ...
  const inspect = (subject: {
    ...
    const body = buildInspectionBody(subject)
    setModalObservation(state.observations[RUNNING_NOTES_KEY] || state.observations[subject.id] || '')
  ...
  const completeInspection = (current: ModalContent, answer?: boolean) => {
    ...
    if (observation) {
      next = {
        ...
        observations: {
          ...next.observations,
          [RUNNING_NOTES_KEY]: observation,
          [current.subjectId]: observation,
        },
      }
    }
    next = record(
  ...
  const incompleteHint = !observationNoteDone
    ? 'Add concise notes about what you observed, then save the popup.'
    ...
    <SceneWrapper showBack backLabel="Back" hideIllustration>
      ...
      >
        ...
        <section className="physical-playground">
          <div className="physical-playground-stage" data-scene-id={node.id} aria-label={`${node.title} workplace stage`}>
            <img
              src={background}
              alt=""
            ...
            {import.meta.env.DEV && node.id !== 'warmup_observation' && (node.background?.coordinateMap || []).map((zone) => (
              <div
                key={zone.label}
        ...
        {!!state.warnings.length && (
          <div style={{ border: '1px solid #B87D6B', background: '#F2EBD9', padding: '0.75rem', fontSize: '0.78rem', lineHeight: 1.45 }}>
            ...
            <div className="physical-playground-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <label className="physical-playground-observation-field">
                ...
                <textarea
                  ...
                  placeholder={modal.observationPlaceholder || 'Add concise notes about what you observed. Keep useful details from earlier observations in this same note.'}
                  rows={4}
                />
              </label>
