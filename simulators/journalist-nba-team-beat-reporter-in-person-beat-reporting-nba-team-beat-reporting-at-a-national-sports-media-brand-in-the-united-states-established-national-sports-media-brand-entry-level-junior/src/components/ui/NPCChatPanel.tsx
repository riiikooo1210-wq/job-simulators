...
interface Props {
  ...
  embedded?: boolean
  /** Prior player artifacts the NPC should use when giving feedback. */
  contextResponses?: ChatContextResponse[]
  /** If set, the NPC writes the first visible message using the contextResponses. */
  aiInitialMessagePrompt?: string
  /** Optional fallback if AI feedback is unavailable. */
...
function buildContextBlock(contextResponses: ChatContextResponse[]) {
  return contextResponses
    .filter((response) => response.content.trim())
    .map((response) => `--- ${response.title} [${response.key}] ---\n${response.content.trim()}`)
    .join('\n\n')
}
...
function LiveSlackMessageRow({ message, npc, playerName }: { message: ChatMessage; npc: NPC; playerName: string }) {
  ...
  const speakerRole = isUser ? 'NBA Beat Reporter' : npc.role
...
export default function NPCChatPanel({
  ...
  embedded = false,
  contextResponses = [],
  aiInitialMessagePrompt,
  aiInitialMessageFallback,
}: Props) {
  const npc = npcs[npcId]
  const conversationKey = sceneId ? `chat:${sceneId}:${aiInitialMessagePrompt ? 'ai-v3:' : ''}${npcId}` : npcId
  ...
  useEffect(() => {
    ...
    if (initialMessages.length > 0) {
      initialMessages.forEach((m) => appendNpcMessage(conversationKey, {
        ...m,
    ...
    if (!aiInitialMessagePrompt) return

    const seedInitialMessage = async () => {
      setError(null)
      if (!contextBlock.trim()) {
        appendNpcMessage(conversationKey, {
          role: 'npc',
          content: aiInitialMessageFallback || 'I do not see the submitted draft in this thread yet. Send me the confirmed angle, what should stay held, and the first follow-up you would make.',
          ts: new Date().toISOString(),
        })
        return
      }
      setLoading(true)
      const initialGoalPrompt = `${aiInitialMessagePrompt}\n\nSUBMITTED WORK TO REVIEW:\n${contextBlock}\n\nThis is the first editor message in the Slack thread, before the reporter has replied. Ask for a response; do not acknowledge a response, do not close the thread, and do not say "thread closed."`
      try {
        ...
        if (aiInitialMessageFallback) {
          appendNpcMessage(conversationKey, {
            role: 'npc',
            content: aiInitialMessageFallback,
            ts: new Date().toISOString(),
          })
        } else {
  ...
  const submit = async (text: string) => {
    ...
    try {
      ...
      const reply = await npcReply({ npcId, history, goalPrompt: contextualGoalPrompt, channel })
      appendNpcMessage(conversationKey, { role: 'npc', content: reply, ts: new Date().toISOString() })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'NPC reply failed')
    } finally {
      ...
      >
        ...
        >
          <span style={{ color: '#fff', fontSize: '0.65rem', fontWeight: 700 }}>
            {getSlackInitials(npc.name) || 'D'}
          </span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '0.875rem', fontWeight: 700, color: '#1d1c1d' }}>{npc.name}</div>
          <div style={{ fontSize: '0.6875rem', color: '#616061', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            Direct message - {npc.role}
          </div>
        </div>
      ...
      {onComplete && embedded && isSlack ? (
        ...
        >
          <button
            ...
            style={{
              backgroundColor: userTurns >= 1 || reachedMax ? '#B87D6B' : '#EFE8D2',
              color: userTurns >= 1 || reachedMax ? '#F7F1E3' : '#8A8176',
              border: '1px solid #CDBF94',
              borderRadius: '4px',
              padding: '0.5rem 0.875rem',
              fontSize: '0.8125rem',
              fontWeight: 700,
              cursor: userTurns >= 1 || reachedMax ? 'pointer' : 'not-allowed',
              fontFamily: 'Inter, system-ui, sans-serif',
            }}
          >
            {completeLabel}
