...
export default function VoiceMeetingScene({ node }: Props) {
  ...
  const statusLabel =
    status === 'connecting' ? (statusDetail ? `Connecting... ${statusDetail}` : 'Connecting...')
  ...
  const meetingInner = (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: isInPerson ? 360 : undefined, color: '#1E1E1A', padding: '0.875rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.125rem', minWidth: 96 }}>
          <div style={{ fontSize: '0.75rem', opacity: 0.72 }}>{statusLabel}</div>
          <div style={{ fontSize: '0.6875rem', opacity: 0.66 }}>
            {Math.min(userTurns, maxTurns)}/{maxTurns} professor turns
          </div>
        </div>
      </div>
  ...
  const inPersonSceneImageSrc = isInPerson ? (node.illustration || `/scenes/${node.id}.png`) : ''
  const inPersonSceneImage = inPersonSceneImageSrc ? (
    <div className="voice-meeting-inperson-scene-image">
      <img
        src={inPersonSceneImageSrc}
        alt={`${node.title} scene`}
        loading="eager"
