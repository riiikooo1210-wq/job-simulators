...
export default function VoiceMeetingScene({ node }: Props) {
  ...
  const callNotesKey = `${node.id}_notes`
  const callNotes = freeTextResponses[callNotesKey] || ''
      ...
      >
        ...
        {(playerGoal || endpoint || successCriteria) && (
          ...
          >
            ...
            {endpoint && <div style={{ marginTop: playerGoal ? '0.375rem' : 0 }}><strong>Completion endpoint: </strong>{endpoint}</div>}
        ...
        >
          <div style={{ fontSize: '0.8125rem', fontWeight: 800, marginBottom: '0.25rem' }}>
            Call notes for the follow-up tracker
          </div>
          <div style={{ fontSize: '0.75rem', lineHeight: 1.5, color: '#5A5145', marginBottom: '0.55rem' }}>
            Jot down staff guidance, follow-up items, and unresolved questions while you talk. You will use these notes to write the pre-app follow-up tracker.
          </div>
          <textarea
            value={callNotes}
            onChange={(event) => setFreeTextResponse(callNotesKey, event.target.value)}
            placeholder="Capture staff guidance, required submittal materials, follow-up items, and open questions from the call."
            rows={4}
            style={{
              width: '100%',
