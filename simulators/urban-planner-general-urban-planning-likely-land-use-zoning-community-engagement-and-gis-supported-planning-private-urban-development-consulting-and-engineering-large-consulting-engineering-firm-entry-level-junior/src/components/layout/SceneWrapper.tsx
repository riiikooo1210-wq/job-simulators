...
export default function SceneWrapper({ children, illustration, showBack = false, hideIllustration = false, backLabel = 'Back to briefing' }: SceneWrapperProps) {
  ...
  const resolvedIllustration = illustration || (currentNodeId ? `/scenes/${currentNodeId}.png` : undefined)

  useLayoutEffect(() => {
    if (!resolvedIllustration || hideIllustration) {
    ...
    probe.src = resolvedIllustration

    return () => {
      cancelled = true
    }
  }, [children, currentNodeId, hideIllustration, resolvedIllustration])

  const shouldShowIllustration = resolvedIllustration && !hideIllustration && imageAvailable && !hasDuplicateIllustration
