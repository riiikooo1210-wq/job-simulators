...
export default function FreeTextScene({ node }: Props) {
  ...
  const interpolationState = { playerName, branchFlags, mcSelections, freeTextResponses: responses }
  ...
  const emailHeaders = {
    from: interpolate(node.emailHeaders?.from || '{{playerName}}', interpolationState),
    to: interpolate(node.emailHeaders?.to || '', interpolationState),
    subject: interpolate(node.emailHeaders?.subject || node.windowTitle || node.title, interpolationState),
  }
  ...
  const tabbedWindowContent = activeAppTab === 'editor' ? (
    ...
    appTabs.map((tab) => (
      activeAppTab === tab.id && (
        ...
        >
          {renderContentWithGlossary(interpolate(tab.content, interpolationState))}
        </div>
      )
    ))
  )
      ...
      >
        ...
        {node.content && (
          <div style={{ fontSize: '0.875rem', lineHeight: 1.7, color: '#333', whiteSpace: 'pre-wrap' }}>
            {renderContentWithGlossary(interpolate(node.content, interpolationState))}
          </div>
        )}
        <div
          style={{
        ...
        >
          {renderContentWithGlossary(interpolate(node.prompt, interpolationState))}
        </div>
