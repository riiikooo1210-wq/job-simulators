...
export default function StructuredEntryScene({ node }: Props) {
  ...
  const [activeAppTab, setActiveAppTab] = useState('form')
  ...
  const localReferenceTabs = node.referenceContent
    ? [{ id: 'reference', label: node.referenceTitle || 'Reference', content: node.referenceContent }]
    : []
  const appTabs = mergeWorkSurfaceTabs(node, localReferenceTabs)
  const titleTabs = appTabs.length > 0
    ? [{ id: 'form', label: node.workSurface?.title || node.windowTitle || node.title }, ...appTabs.map((tab) => ({ id: tab.id, label: tab.label }))]
    : undefined
  ...
  const tabbedWindowContent = activeAppTab === 'form' ? (
    formContent
  ) : (
    appTabs.map((tab) => (
      activeAppTab === tab.id && (
      ...
      >
        ...
        {node.content && (
          <div style={{ fontSize: '0.875rem', lineHeight: 1.7, color: '#333', whiteSpace: 'pre-wrap' }}>
            {renderContentWithGlossary(interpolate(node.content, { playerName, branchFlags, mcSelections, freeTextResponses: responses }))}
          </div>
        )}
        <div
          style={{
        ...
        >
          {renderContentWithGlossary(interpolate(node.prompt, { playerName, branchFlags, mcSelections, freeTextResponses: responses }))}
        </div>
