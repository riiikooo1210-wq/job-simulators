...
type BriefingNoteWindow = NonNullable<BriefingNode['noteWindow']>
...
function BriefingSplitDesktopOverlay({ children }: { children: ReactNode }) {
  return (
    <div
      style={{
        position: 'relative',
        width: 'calc(100% + 6rem)',
        minHeight: 'clamp(28rem, 48vw, 34rem)',
        marginLeft: '-3rem',
        marginRight: '-3rem',
    ...
    >
      ...
      >
        <div style={{ width: '72%', height: '82%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          {children}
        </div>
      </div>
    </div>
...
function BriefingNotesWindow({ noteWindow }: { noteWindow: BriefingNoteWindow }) {
  const responses = useGameStore((s) => s.freeTextResponses)
  const setFreeTextResponse = useGameStore((s) => s.setFreeTextResponse)
  const value = responses[noteWindow.bindingKey] || ''
  const wordCount = value.trim() ? value.trim().split(/\s+/).length : 0

  return (
    <LaptopFrame variant="notion" title={noteWindow.title || 'Meeting Prep Notes'} fill>
      <div
        style={{
          height: '100%',
          minHeight: 0,
      ...
      >
        {noteWindow.instructions && (
          ...
          >
            {renderContentWithGlossary(noteWindow.instructions)}
          </div>
        )}
        <textarea
          aria-label={noteWindow.title || 'Meeting prep notes'}
          value={value}
          onChange={(e) => setFreeTextResponse(noteWindow.bindingKey, e.target.value)}
          placeholder={noteWindow.placeholder || 'Jot the few points you want ready for the next meeting.'}
          style={{
            flex: 1,
            minHeight: 0,
            width: '100%',
        ...
        />
        <div style={{ fontSize: '0.7rem', color: '#5F6B63', textAlign: 'right' }}>
...
function BriefingReferenceWindow({
  title,
  content,
  variant,
  noteWindow,
  ...
  noteWindow?: BriefingNode['noteWindow']
}) {
  const referenceWindow = (
    <LaptopFrame variant={variant} title={title} scrollable fill>
        <article
          style={{
            maxWidth: noteWindow ? 'none' : '44rem',
            margin: '0 auto',
            backgroundColor: '#FFFDF7',
            border: '1px solid #D8CDB8',
            boxShadow: '0 2px 10px rgba(30,30,26,0.08)',
            padding: noteWindow ? '1rem' : '1.4rem 1.6rem',
            color: '#1E1E1A',
            minHeight: '100%',
          }}
        >
          ...
          <div style={{ fontSize: '0.82rem', lineHeight: 1.62, whiteSpace: 'pre-wrap' }}>
            {renderContentWithGlossary(content)}
          </div>
        </article>
      </LaptopFrame>
  ...
  if (noteWindow) {
    return (
      <BriefingSplitDesktopOverlay>
        ...
        >
          {referenceWindow}
          <BriefingNotesWindow noteWindow={noteWindow} />
        </div>
      </BriefingSplitDesktopOverlay>
    )
  }
  ...
  return (
    <DesktopOverlay width="72%" height="82%">
      {referenceWindow}
    </DesktopOverlay>
  )
}
...
function renderSourceInboxFilePreview(file: SourceInboxFile | null, compact = false) {
  if (!file) {
    return (
      <div
        style={{
...
function SourceWorkspaceBriefing({ node }: { node: BriefingNode }) {
  ...
  const activeFile = activeApp?.files?.find((file) => file.id === activeFileId) || null
