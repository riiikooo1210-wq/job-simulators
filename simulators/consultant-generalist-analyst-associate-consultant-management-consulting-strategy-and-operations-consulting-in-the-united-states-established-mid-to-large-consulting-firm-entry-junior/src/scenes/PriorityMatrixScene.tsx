...
export default function PriorityMatrixScene({ node }: { node: PriorityMatrixNode }) {
  ...
  const requireAllItemsPlaced = node.requireAllItemsPlaced === true
  const hasRequiredPlacements = requireAllItemsPlaced
    ? placedItemIds.length === node.items.length
    : placedItemIds.length >= 1
  const canSubmit = hasRequiredPlacements && rationale.trim().length >= 20
  const titleTabs = node.referenceContent
    ? [
        { id: 'matrix', label: 'Matrix' },
        { id: 'reference', label: node.referenceTitle ?? 'Reference' },
      ]
    : undefined
  ...
  const referenceContent = (
    ...
    >
      {node.referenceTitle && (
        <h2 style={{ fontSize: '0.95rem', lineHeight: 1.3, margin: '0 0 0.75rem', color: '#3F605C' }}>
          {node.referenceTitle}
        </h2>
      )}
    ...
    <SceneWrapper illustration={node.illustration} showBack backLabel="Back" hideIllustration={hasWorkSurfaceVisual(node)}>
      ...
      >
        ...
        <WorkSurfaceFrame
          node={node}
          variant="miro"
          title={node.workSurface?.title || node.windowTitle || node.title}
          titleTabs={titleTabs}
          activeTitleTabId={activeAppTab}
          onTitleTabChange={setActiveAppTab}
        >
            {activeAppTab === 'reference' ? referenceContent : (
            <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
                  <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%', padding: '0.6rem', gap: '0.75rem' }}>

                {/* Unplaced items sidebar */}
                {unplacedItems.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem', padding: '0.4rem 0.6rem', backgroundColor: '#EFE8D2', border: '1px solid #CDBF94', borderRadius: '6px' }}>
                    <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#3F605C', width: '100%', marginBottom: '0.125rem' }}>DRAG EVERY ITEM ONTO THE MATRIX</span>
                ...
                <div ref={matrixRef} style={{ flex: '0 0 auto', display: 'flex', flexDirection: 'column', height: 'clamp(330px, 44vh, 430px)', minHeight: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'stretch', flex: 1, gap: 0 }}>
                    {/* Y axis */}
                    <div style={{ width: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: '20px', overflow: 'hidden' }}>
                      <span style={{ fontSize: '0.58rem', color: '#3F605C', fontWeight: 700, writingMode: 'vertical-rl', transform: 'rotate(180deg)', whiteSpace: 'nowrap' }}>
                        {node.yAxis.label}
                      </span>
                    </div>
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                      {/* 2x2 grid */}
                      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', border: '2px solid #CDBF94', backgroundColor: '#EFE8D2' }}>
                        {quadrants.map((q) => (
              ...
              <DragOverlay>
                {activeItem ? (
                  <div style={{
                    ...
                    fontFamily: 'Inter, system-ui, sans-serif',
                    color: '#fff',
                    boxShadow: '0 8px 20px rgba(30,30,26,0.28)',
                    cursor: 'grabbing',
                    whiteSpace: 'nowrap',
                  }}>
                    {activeItem.label}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', flex: '0 0 auto', paddingTop: '0.5rem', borderTop: '1px solid #CDBF94', backgroundColor: '#F7F1E3' }}>
          ...
          <textarea
            ...
            placeholder={node.rationalePlaceholder ?? 'e.g. I would handle the metric-definition issue first because it has high decision impact and could change the recommendation before manager review...'}
            rows={3}
            style={{
          ...
          {!hasRequiredPlacements && (
            <span style={{ fontSize: '0.7rem', color: '#B87D6B' }}>
              {requireAllItemsPlaced
                ? `Place all ${node.items.length} items before submitting.`
                : 'Place at least one item before submitting.'}
            </span>
          )}
          <ActionButton
            text="Submit"
