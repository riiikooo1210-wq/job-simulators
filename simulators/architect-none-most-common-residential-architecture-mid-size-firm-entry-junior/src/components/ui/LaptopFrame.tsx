...
export type LaptopFrameVariant = 'doc' | 'email' | 'email-read' | 'slack' | 'figma' | 'notion' | 'spreadsheet' | 'code' | 'miro' | 'kanban' | 'meeting' | 'bim'
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* BIM: project view title */}
        {!hasTitleTabs && variant === 'bim' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, marginLeft: '0.5rem' }}>
            <span style={{ fontSize: '0.72rem', color: '#3F605C', fontWeight: 800 }}>Maple Street Addition</span>
            <span style={{ fontSize: '0.72rem', color: '#6A604B', fontWeight: 600 }}>{displayTitle}</span>
          </div>
        )}
      ...
      {variant === 'miro' && (
        ...
        >
          {['Select', 'Hand', 'Sticky', 'Shape', 'Text', 'Arrow'].map((tool) => (
            <span key={tool} style={{ fontSize: '0.65rem', color: '#7B5EA7', cursor: 'default', userSelect: 'none' }}>{tool}</span>
          ))}
        </div>
      )}
      ...
      <div
        style={{
          padding: variant === 'slack' || variant === 'figma' || variant === 'kanban' || variant === 'meeting' || variant === 'bim' ? '0' : '1.25rem',
          backgroundColor: contentBg,
          flex: '1 1 auto',
          minHeight: 0,
