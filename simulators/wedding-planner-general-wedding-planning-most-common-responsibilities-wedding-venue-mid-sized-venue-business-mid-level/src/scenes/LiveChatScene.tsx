...
export default function LiveChatScene({ node }: Props) {
  ...
  const conversation = useGameStore((s) => s.npcConversations[node.npcId] || [])
  ...
  const externalComplete = Boolean(appWindow && node.type === 'slack_thread')
  const requiredSlackReplyTargets = Array.from(new Set(
    (node.initialMessages || [])
      .map((message) => message.npcName)
      .filter((name): name is string => {
        if (!name) return false
        return !/^Maya Patel\b/.test(name)
      })
  ))
  const repliedSlackTargets = new Set(
    conversation
      .filter((message) => message.role === 'user' && message.npcName)
      .map((message) => message.npcName!)
  )
  const externalCompleteReady = externalComplete
    ? (requiredSlackReplyTargets.length === 0
      ? userTurns >= 1 || reachedMax
      : requiredSlackReplyTargets.every((name) => repliedSlackTargets.has(name)))
    : true
  ...
  const chatPanel = (
    <NPCChatPanel
      ...
      sceneId={node.id}
      onComplete={externalComplete ? undefined : () => goNext(node)}
      completeLabel="Submit & continue"
      embedded={!!appWindow}
      conversationTitle={node.workSurface?.title || node.windowTitle}
    />
  )

  return (
    <SceneWrapper illustration={node.illustration} showBack backLabel="Back" hideIllustration={Boolean(appWindow || node.workSurface)}>
      <motion.div
      ...
      >
        ...
        {appWindow ? (
          <WorkSurfaceFrame node={node} variant={appWindow} title={node.workSurface?.title || node.windowTitle}>
            {chatPanel}
          </WorkSurfaceFrame>
        ) : (
          chatPanel
        ...
        {externalComplete && (
          <ActionButton
            ...
            onClick={() => goNext(node)}
            disabled={!externalCompleteReady}
            variant={externalCompleteReady ? 'primary' : 'secondary'}
          />
        )}
        {import.meta.env.DEV && (
