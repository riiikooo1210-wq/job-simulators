...
import TimelineDocumentTable from './TimelineDocumentTable'
...
function isTimelineDocument(file: SourceInboxFile, columns: string[]) {
  return file.id.includes('timeline') && columns.includes('time') && columns.includes('timeline_item')
}
...
function renderSourceFilePreview(file: SourceInboxFile | null, ctx: SourceWorkspaceContext) {
  ...
  if (rows.length > 0 && columns.length > 0) {
    if (isTimelineDocument(file, columns)) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          ...
          <TimelineDocumentTable rows={rows} />
        </div>
      )
    }
